...
STALE_RELEASE_PROMISE = re.compile(
    r"schema[- ]only|deferred\s+to\s+v\d|lands\s+in\s+v\d|ships\s+in\s+v\d|post-v\d",
    re.IGNORECASE,
)
...
def _schema_descriptions() -> list[tuple[str, int, str]]:
...
def test_schema_descriptions_have_no_expired_release_promises() -> None:
    ...
    offenders = [
        ...
        if STALE_RELEASE_PROMISE.search(description)
    ]
    assert offenders == []
...
def test_cli_help_has_no_expired_promises_or_internal_phase_labels() -> None:
    ...
    offenders = [
        ...
        if STALE_RELEASE_PROMISE.search(text) or INTERNAL_RELEASE_PHASE.search(text)
    ]
    assert offenders == []
