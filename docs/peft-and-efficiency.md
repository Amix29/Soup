...
RoPE scaling is applied to the Transformers model config before model construction for both `task: sft` and `task: pretrain`. Existing model-native parameters such as `rope_theta` are preserved. `longrope` additionally requires a checkpoint that already ships its learned `short_factor` and `long_factor` vectors; Soup refuses to invent those model-specific values.
